...
def data(task, input_type):
    if input_type == 'clean':
        fi = np.transpose(loadmat('./b/data_clean.mat')['data'])  # clean data

    if input_type == 'noise':
        fi = np.transpose(loadmat('./b/data_noised.mat')['data_noised'])  # noised data

    fo = np.transpose(loadmat('./b/data_clean.mat')['data'])  # output data
